...
public class MetaMapper
{
    ...
    public static ClassMap<Metadata> Map(Metadata[] entries)
    {
        ...
        if (entries.Any(entry => entry.Donor != null))
        {
            ...
            MapProperty(map, entries, entry => entry.Donor.SteroidsBaseline, "donor_steroids_baseline");
            MapProperty(map, entries, entry => entry.Donor.KpsBaseline, "donor_kps_baseline");
        }
        ...
        if (entries.Any(entry => entry.Image != null))
        {
            ...
            if (entries.Any(entry => entry.Image.Mri != null))
            {
                MapProperty(map, entries, entry => entry.Image.Mri.WholeTumor, "mri_whole_tumor");
                MapProperty(map, entries, entry => entry.Image.Mri.ContrastEnhancing, "mri_contrast_enhancing");
                MapProperty(map, entries, entry => entry.Image.Mri.NonContrastEnhancing, "mri_non_contrast_enhancing");
            }
        }
        ...
        if (entries.Any(entry => entry.Specimen != null))
        {
            ...
            {
                MapProperty(map, entries, entry => entry.Specimen.Line.CellsSpecies, "line_cells_species");
                MapProperty(map, entries, entry => entry.Specimen.Line.CellsType, "line_cells_type");
                MapProperty(map, entries, entry => entry.Specimen.Line.CellsCultureType, "line_cells_culture_type");
            }
